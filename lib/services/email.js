...
// Wrapper function for backwards compatibility with existing imports
export const sendBookingConfirmation = async (emailData) => {
  console.log('📧 sendBookingConfirmation called with data:', JSON.stringify(emailData, null, 2))
  
  // Transform emailData to match the expected order format
  const order = {
    id: emailData.orderId,
    passenger: {
      fullName: emailData.passengerName,
      email: emailData.passengerEmail,
      phone: 'N/A' // Default value if not provided
    },
    totalPrice: emailData.totalPrice,
    currency: emailData.currency,
    paymentMethod: emailData.paymentMethod,
    createdAt: emailData.bookingDate
  }
  
  // Transform route data if provided
  let route = null
  if (emailData.route) {
    route = {
      fromCity: emailData.route.fromCity?.names?.en || 'Departure City',
      toCity: emailData.route.toCity?.names?.en || 'Destination City',
      departureTime: emailData.route.departureTime,
      arrivalTime: emailData.route.arrivalTime,
      carrier: emailData.route.carrier?.name || 'Bus Carrier',
      isDirect: emailData.route.isDirect,
      duration: emailData.route.duration
    }
  }
  
  console.log('📧 Transformed order data:', JSON.stringify(order, null, 2))
  console.log('📧 Transformed route data:', JSON.stringify(route, null, 2))
  
  return await emailService.sendOrderConfirmation(order, route)
}
